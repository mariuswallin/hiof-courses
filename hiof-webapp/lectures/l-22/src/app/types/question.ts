...
export interface Question {
  ...
  publishedAt?: Date; // Optional field for tracking publication time
  deletedAt?: Date; // Optional field for tracking deletion time (soft delete)
}
...
interface DbQuestion {
  id: string; // Readonly property to prevent modification
  question: string;
  status: QuestionStatus;
  createdAt: Date;
  authorId: number | null;
  updatedAt: Date | null; // Optional field for tracking updates
  publishedAt: Date | null; // Optional field for tracking publication time
  deletedAt: Date | null; // Optional field for tracking deletion time (soft delete)
}

export interface QuestionWithAnswers extends DbQuestion {
  answers: DbAnswer[];
}
